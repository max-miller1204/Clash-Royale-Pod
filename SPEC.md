...
_Wave 2I executed 2026-05-03 on branch swarm/wave-2i-drop-rate-fix; chunks wave-2i-drop-rate-fix (destroyed-tower bookend → delta=0 when a tower reads None across every frame in window; gap-tolerant HP-bar reader bridging gaps ≤ MIN_BRIDGE_SEG_PX=10 px; mask-threshold loosening deferred; calibration regression gate added — all four fixture-frame towers ≤2.5% MAE; tests 90 → 100). Code only — brev train run on arena_23+ pool pending; metrics (drop rate, ρ, Δρ vs 2H baseline 0.078) to land in follow-up commit on this branch before merge. PR https://github.com/max-miller1204/Clash-Royale-Pod/pull/36; landed_on_main: no; brev_metrics: pending._
