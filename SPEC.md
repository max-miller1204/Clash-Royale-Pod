...
_Wave 2J executed 2026-05-04 on branch swarm/wave-2j-feature-audit; chunks wave-2j-feature-audit (audit-only — extracted LightGBM `gain` + `split` importances from `output/models/ev_wave2i.joblib` via `scripts/audit_ev_features.py`; max(gain) = 4.99B held by `start_enemy_total_princess_hp`, lowest feature `n_friendly_cards` at **7.14 % × max(gain) / 169 splits** — both far above the `gain < 1%` AND `split < 5` drop thresholds. **No drops**: `interaction_features` untouched, no brev run, **$0 cost**, **Δρ N/A** by spec. Wave-2F HP-context pair dominates importance — top two rows at 100 % / 86.34 %, consistent with the +0.116 ρ jump landing in 2I once destroyed-tower bookends stopped dropping those rows wholesale. 11-feature baseline confirmed fully utilized; 2J' (feature add) is next. PR https://github.com/max-miller1204/Clash-Royale-Pod/pull/39; landed_on_main: yes (8623b73)._

_Wave 2J' executed 2026-05-05 on branch swarm/wave-2j-prime-feature-add; chunks wave-2j-prime-feature-add (added pre_window_{friendly,enemy}_hp_delta_30s and start_seconds to Interaction; extended build_interactions with fps param and 30-s pre-window HP-delta lookup; added 4 feature keys to interaction_features: top_friendly_x_top_enemy, pre_window_friendly_hp_delta_30s, pre_window_enemy_hp_delta_30s, time_pressure_mode; pipeline.analyze_replay forwards fps=replay.fps; tests +24, suite 88 → 112). Brev massedcompute_A6000_plus 13h38m / ~$9.27: drop rate 13% → 13% (identical, same data + reader), train/holdout 16,490 / 4,030 (frozen-holdout match), per_card_stats 72 → 72, MAE 326.82 → **318.44** (−2.6%), holdout ρ +0.194 → **+0.223**, **Δρ = +0.029** (above the 0.02 noise band, well clear of the −0.02 stop). Importance audit on the new model: `pre_window_enemy_hp_delta_30s` took the #1 slot (100% × max gain), `pre_window_friendly_hp_delta_30s` #2 (76.5%), `top_friendly_x_top_enemy` #5 (58.8%); `time_pressure_mode` was inert (zero gain / zero splits — DROP-eligible by the wave-2J audit rule, retained this wave per the additive-only discipline). Wave-2.5 stop conditions all clear; per the design doc, wave 2K (model class A/B + hyperparameter sweep) runs because Δρ is small. PR https://github.com/max-miller1204/Clash-Royale-Pod/pull/41; landed_on_main: yes (4ed08c3)._

_Wave 2K executed 2026-05-15 on branch swarm/wave-2k-model-class-ab; chunks wave-2k-model-class-ab (added scripts/cv_sweep.py 5-fold CV harness + crpod train --save-data flag for offline CV; 220-config sweep across LightGBM 108 / Ridge 4 / XGBoost 108 on the frozen-holdout train fold; CV winner picked by mean fold-ρ, holdout touched once, re-shuffled-split leak check). Brev massedcompute_A6000_plus 13h21m (Stage A train+save-data 12h37m, Stage B sweep 43m) / ~$9.10 compute (run total ≈ $19.80 incl. ~$10.70 prior-agent boot-idle overhead): same 15-feature set / same data / same frozen-holdout split as 2J' (drop rate 13%, train/holdout 16,490 / 4,030, per_card_stats 72 — all identical). **CV winner: LightGBM `{n_estimators: 100, learning_rate: 0.02, num_leaves: 31, min_child_samples: 10}`, CV mean ρ +0.2428 ± 0.0149**; beat best XGBoost (+0.2391, gap inside one fold's std) and best Ridge (+0.2022). Holdout MAE 318.44 → **295.61** (−7.2%), holdout ρ +0.223 → **+0.2302**, **Δρ = +0.0072** (below the 0.02 noise band — not a regression, but tuning is not the lever). Re-shuffled-split ρ +0.2386, gap 0.0084 < 0.05 — no holdout leak. Model-class A/B answered: GBDTs beat linear, LightGBM ≈ XGBoost, no untuned model-class win exists. Saturation signal — remaining ranking signal is feature/label-bound, not learner-bound; first sub-noise wave (2J' cleared the band at +0.029) so the "twice consecutively" stop has not strictly fired, flagged for operator. PR https://github.com/max-miller1204/Clash-Royale-Pod/pull/<TBD>; landed_on_main: no._
