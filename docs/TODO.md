...
- [x] **Expand `CARD_COSTS` to all 159 cards.** `src/crpod/constants.py` now covers 116 base cards — all standard troops, buildings, spells, and 8 champions — sourced from the `cr-csv` mirror of Supercell's `spells_*.csv` with post-2023 rebalances and 2024–2026 additions applied by hand. Evolution variants share the base cost and aren't listed separately. A handful of very recent champions (boss_bandit, rune_giant, spirit_empress, terry) are still omitted — they'll fall back to `default=3` until their costs are confirmed. Name mapping landed in `src/crpod/detection/cards.py` (Phase 1). Phase 2 is populating `KATACR_TO_CARD` and `KATACR_NON_CARD` against the snapshot — run `scripts/dump_katacr_classes.py` on a machine with `output/models/crpod_v1_best.pt`, commit the resulting `src/crpod/detection/katacr_classes.txt`, then fill in the mapping rows until `pytest tests/test_cards.py` passes (no skips).
