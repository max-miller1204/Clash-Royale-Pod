...
- [x] **Expand `CARD_COSTS` to all 159 cards.** `src/crpod/constants.py` now covers 116 base cards — all standard troops, buildings, spells, and 8 champions — sourced from the `cr-csv` mirror of Supercell's `spells_*.csv` with post-2023 rebalances and 2024–2026 additions applied by hand. Evolution variants share the base cost and aren't listed separately. A handful of very recent champions (boss_bandit, rune_giant, spirit_empress, terry) are still omitted — they'll fall back to `default=3` until their costs are confirmed. [x] Name-mapping layer for KataCR class names (`the-log`, `spear-goblin`, `phoenix-egg`, etc.) lands in `crpod.constants.normalize_card_name` + `_KATACR_ALIASES`; `card_cost()` now routes through it so kebab-case YOLO labels actually hit `CARD_COSTS`. Use `scripts/audit_card_names.py` against trained weights to find any remaining unmapped classes.
